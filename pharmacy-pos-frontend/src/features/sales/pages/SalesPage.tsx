...
import { Sale, SaleStatus, PaymentMethod } from '../types/sale.types.js';
...
import { ReceiptText, Search, Eye, ChevronLeft, ChevronRight, ShoppingBag } from 'lucide-react';
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-slate-200/80 dark:border-[#1E293B]">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white tracking-tight flex items-center gap-2.5">
            <ReceiptText className="w-6 h-6 text-sky-600 dark:text-sky-400" />
            <span>سجل فواتير المبيعات</span>
          </h1>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
            سجل التدقيق لكافة المعاملات المالية وفواتير البيع الصادرة من الكاشير
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <Card className="rounded-3xl p-12 text-center">
          <EmptyState
            ...
            description="لم يتم العثور على أي فواتير تطابق معايير البحث الحالية."
          />
        </Card>
      ) : (
            ...
            <table className="w-full text-xs text-start">
              <thead className="bg-slate-50 dark:bg-[#0B0F17]/70 border-b border-slate-100 dark:border-[#1E293B] text-slate-500 dark:text-slate-400 font-bold uppercase">
                <tr>
                  ...
                  <th className="py-3.5 px-4 text-start">الكاشير</th>
                  <th className="py-3.5 px-4 text-start">الأصناف</th>
                  <th className="py-3.5 px-4 text-start">إجمالي الفاتورة</th>

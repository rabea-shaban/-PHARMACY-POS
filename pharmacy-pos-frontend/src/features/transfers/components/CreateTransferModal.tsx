...
export const CreateTransferModal: React.FC<CreateTransferModalProps> = ({
  ...
  useEffect(() => {
    const fetchBranches = async () => {
      try {
        const res = await branchesApi.getBranches({ isActive: true, limit: 100 });
        setBranches(res.items);

        // Lock fromBranchId directly to the current logged-in active branch
        const myBranch =
          activeBranch ||
          user?.branch ||
          (user?.branchId ? res.items.find((b) => b.id === user.branchId) : null) ||
          res.items[0];

        if (myBranch) {
          setFromBranchId(myBranch.id);
          const others = res.items.filter((b) => b.id !== myBranch.id);
          if (others.length > 0) {
            setToBranchId(others[0].id);
          }
        }
      } catch (err) {
  ...
  }, [isOpen, presetProduct, activeBranch, user?.branchId, user?.branch]);
            ...
            <div>
              <label className="block font-bold text-slate-700 dark:text-slate-200 mb-1.5 flex items-center justify-between">
                ...
                <span className="text-[10px] font-semibold text-slate-400 flex items-center gap-1">
                  ...
                  {isAr ? 'فرعك الحالي' : 'Current Branch'}
                </span>
              </label>

              {(() => {
                const currentSourceBranch =
                  branches.find((b) => b.id === fromBranchId) || activeBranch || user?.branch;
                return (
                  <div className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white font-bold flex items-center justify-between shadow-xs">
                    <div className="flex items-center gap-2 truncate">
                      <Store className="w-4 h-4 text-sky-600 dark:text-sky-400 shrink-0" />
                      <span className="truncate">
                        {currentSourceBranch?.name || (isAr ? 'فرعك الحالي' : 'Active Branch')}
                      </span>
                      {currentSourceBranch?.code && (
                        <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-500">
                          {currentSourceBranch.code}
                        </span>
                      )}
                    </div>
            ...
            <div>
              ...
              {(() => {
                ...
                if (destinationBranches.length === 0) {
                  return (
                    <div className="w-full px-3 py-2.5 rounded-xl border border-dashed border-amber-300 dark:border-amber-800 bg-amber-50/50 dark:bg-amber-950/30 text-amber-700 dark:text-amber-300 text-[11px] font-bold">
                      {isAr ? 'لا توجد فروع أخرى متاحة للتحويل إليها' : 'No other branches available'}
                    </div>
                  );
                }
                return (
                  <select
                    ...
                    className="w-full px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 font-bold focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500"
                    required
                  >

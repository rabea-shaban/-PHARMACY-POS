import { commissionsRepository, CommissionsRepository } from './commissions.repository.js';
import { auditService, AuditService } from '../audit/audit.service.js';
import { getPaginationMeta } from '../../utils/pagination.util.js';
import {
  CreateCommissionRuleDTO,
  UpdateCommissionRuleDTO,
  CommissionTransactionQueryDTO,
} from './commissions.validator.js';
import {
  CommissionRuleResponse,
  CommissionTransactionResponse,
  CommissionTransactionQueryFilters,
  PaginatedCommissionTransactionsResponse,
  CommissionSummaryResponse,
} from './commissions.types.js';
import { NotFoundError } from '../../utils/errors.js';

function formatRule(raw: any): CommissionRuleResponse {
  return {
    id: raw.id,
    name: raw.name,
...
function formatTransaction(raw: any): CommissionTransactionResponse {
  return {
    id: raw.id,
    userId: raw.userId,
...
export class CommissionsService {
  constructor(
    private readonly repo: CommissionsRepository = commissionsRepository,
    private readonly audit: AuditService = auditService
  ) {}

  async getRules(): Promise<CommissionRuleResponse[]> {
    const rules = await this.repo.findRules();
    return rules.map(formatRule);
  }

  async getRuleById(id: string): Promise<CommissionRuleResponse> {
  ...
  async getActiveRule(): Promise<CommissionRuleResponse | null> {
    const rule = await this.repo.findActiveRule();
    return rule ? formatRule(rule) : null;
  }

  async createRule(input: CreateCommissionRuleDTO, actorId?: string): Promise<CommissionRuleResponse> {
  ...
  async updateRule(id: string, input: UpdateCommissionRuleDTO, actorId?: string): Promise<CommissionRuleResponse> {
    ...
    if (input.fixedAmount !== undefined) updateData.fixedAmount = input.fixedAmount;
  ...
  async getTransactions(filters: CommissionTransactionQueryFilters): Promise<PaginatedCommissionTransactionsResponse> {
  ...
  async getStaffTransactions(userId: string, filters: CommissionTransactionQueryFilters): Promise<PaginatedCommissionTransactionsResponse> {
    return this.getTransactions({ ...filters, userId });
  }

  async getSummary(startDate?: string, endDate?: string): Promise<CommissionSummaryResponse> {

...
export class SalesRepository {
  ...
  async findMany(filters: SaleQueryFilters) {
  ...
  async createSaleAtomic(plan: AtomicCheckoutPlan) {
        ...
        if (item.batchId) {
          ...
          if (updateResult.count === 0) {
            ...
            throw new BadRequestError(
              `Insufficient stock for batch '${batch?.batchNumber || item.batchId}'. Requested ${item.quantity}, Available: ${batch?.quantity || 0}`
            );
          }
      ...
      if (plan.customerId) {
        ...
        if (plan.loyalty?.redeemPoints && plan.loyalty.redeemPoints > 0) {
          if (currentPoints < plan.loyalty.redeemPoints) {
            throw new BadRequestError(
              `Insufficient loyalty points. Requested: ${plan.loyalty.redeemPoints}, Available: ${currentPoints}`
            );
          }

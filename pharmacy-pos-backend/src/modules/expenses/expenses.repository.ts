...
export class ExpensesRepository {
  ...
  async findMany(filters: ExpenseQueryFilters) {
  ...
  async getSummary(startDate?: Date, endDate?: Date) {
    ...
    let totalExpenses = 0;
    const catMap = new Map<ExpenseCategory, { totalAmount: number; count: number }>();
    const pmMap = new Map<PaymentMethod, { totalAmount: number; count: number }>();

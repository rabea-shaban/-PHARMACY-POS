...
export class PurchasesRepository {
  ...
  async findMany(filters: PurchaseQueryFilters) {
  ...
  async receiveAtomic(
    purchaseId: string,
    overrides?: ReceivePurchaseItemData[],
    actorId?: string | null
  ) {
    return prisma.$transaction(async (tx) => {
      ...
      for (const item of purchase.items) {
        const itemOverride = overrides?.find(
          (o) => o.itemId === item.id || o.productId === item.productId
        );
  ...
  async cancelAtomic(purchaseId: string, reason?: string) {
    return prisma.$transaction(async (tx) => {
      const purchase = await tx.purchase.findUnique({
        where: { id: purchaseId },
      });
      ...
      if (purchase.status === 'RECEIVED') {
        throw new BadRequestError(
          'Cannot cancel a RECEIVED purchase directly. Received purchases have already increased inventory. Use inventory adjustment or return workflows.'
        );
      }

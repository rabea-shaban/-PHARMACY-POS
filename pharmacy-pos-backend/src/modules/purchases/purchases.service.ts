import { purchasesRepository, PurchasesRepository } from './purchases.repository.js';
import { suppliersService, SuppliersService } from '../suppliers/suppliers.service.js';
...
function formatPurchase(raw: any): PurchaseResponse {
  return {
    id: raw.id,
...
export class PurchasesService {
  constructor(
    private readonly repo: PurchasesRepository = purchasesRepository,
    private readonly suppliers: SuppliersService = suppliersService,
    private readonly products: ProductsService = productsService,
    private readonly audit: AuditService = auditService
  ) {}

  async getPurchases(filters: PurchaseQueryFilters): Promise<PaginatedPurchasesResponse> {
  ...
  async createPurchase(input: CreatePurchaseDTO, actorId: string): Promise<PurchaseResponse> {
  ...
  async updatePurchase(
    ...
    if (existing.status !== 'PENDING') {
      throw new BadRequestError(
        `Cannot edit purchase with status '${existing.status}'. Only PENDING purchases can be modified.`
      );
    }
  ...
  async receivePurchase(
    ...
    if (existing.status !== 'PENDING') {
      throw new BadRequestError(
        `Cannot receive purchase with status '${existing.status}'. Only PENDING purchases can be received.`
      );
    }

...
import { TransferRequest, TransferStatus, Prisma } from '@prisma/client';
...
export class TransfersRepository {
  ...
  async findByTransferNumber(transferNumber: string): Promise<any | null> {
    return prisma.transferRequest.findUnique({
  ...
  async dispatchAtomic(id: string, dispatchedById: string): Promise<any> {
    return prisma.$transaction(async (tx) => {
        ...
        if (!sourceBatch) {
          ...
          if (!sourceBatch) {
            throw new BadRequestError(
              `Insufficient stock for product '${item.product.name}' in source branch '${transfer.fromBranch.name}'`
            );
          }
          ...
          if (sourceBatch.quantity < item.quantity) {
            throw new BadRequestError(
              `Insufficient stock in batch '${sourceBatch.batchNumber}' for product '${item.product.name}'. Available: ${sourceBatch.quantity}, Requested: ${item.quantity}`
            );
          }
        }

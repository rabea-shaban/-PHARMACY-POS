...
export class InventoryRepository {
  ...
  async findMany(filters: InventoryTransactionQueryDTO) {
  ...
  async findMatrix(query: InventoryMatrixQueryDTO) {
    ...
    const matrixItems = productsResult.items.map((prod) => {
      const branchStockMap: Record<string, { stock: number; batchesCount: number; nearestExpiry: Date | null }> = {};

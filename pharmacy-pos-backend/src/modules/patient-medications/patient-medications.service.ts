import {
  patientMedicationsRepository,
  PatientMedicationsRepository,
} from './patient-medications.repository.js';
import {
...
function parseDosageTimes(val?: string | null): string[] {
  if (!val) return [];
  try {
...
function formatPatientMedication(med: any) {
  return {
    ...med,
...
export class PatientMedicationsService {
  constructor(
    private readonly repo: PatientMedicationsRepository = patientMedicationsRepository,
    private readonly audit: AuditService = auditService
  ) {}

  async getMedications(query: PatientMedicationQueryDTO) {
  ...
  async getCustomerMedications(customerId: string) {
  ...
  async lookupPatientsByProduct(productId: string) {
    if (!productId) {
  ...
  async createMedication(data: CreatePatientMedicationDTO, prescribedById: string): Promise<PatientMedication> {
  ...
  async updateMedication(id: string, data: UpdatePatientMedicationDTO, actorId?: string): Promise<PatientMedication> {
  ...
  async deactivateMedication(id: string, actorId?: string): Promise<PatientMedication> {
    const existing = await this.repo.findById(id);
    if (!existing) {
      throw new NotFoundError(`Patient medication record with ID '${id}' not found`);
    }

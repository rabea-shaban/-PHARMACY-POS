import { saleReturnsRepository, SaleReturnsRepository, AtomicReturnPlan } from './sale-returns.repository.js';
import { auditService, AuditService } from '../audit/audit.service.js';
import { getPaginationMeta } from '../../utils/pagination.util.js';
import { CreateSaleReturnDTO, SaleReturnQueryDTO } from './sale-returns.validator.js';
import {
  SaleReturnResponse,
  SaleReturnQueryFilters,
  PaginatedSaleReturnsResponse,
} from './sale-returns.types.js';
import { NotFoundError, BadRequestError } from '../../utils/errors.js';

function formatSaleReturn(raw: any): SaleReturnResponse {
  return {
    id: raw.id,
...
export class SaleReturnsService {
  constructor(
    private readonly repo: SaleReturnsRepository = saleReturnsRepository,
    private readonly audit: AuditService = auditService
  ) {}

  async getSaleReturns(filters: SaleReturnQueryFilters): Promise<PaginatedSaleReturnsResponse> {
  ...
  async getSaleReturnById(id: string): Promise<SaleReturnResponse> {
  ...
  async getReturnsBySaleId(saleId: string): Promise<SaleReturnResponse[]> {
    const returns = await this.repo.findBySaleId(saleId);
    return returns.map(formatSaleReturn);
  }

  async createSaleReturn(input: CreateSaleReturnDTO, processedById: string): Promise<SaleReturnResponse> {
    ...
    const planItems: AtomicReturnPlan['items'] = [];
    ...
    for (const item of input.items) {
      ...
      if (item.quantity > remainingReturnable) {
        throw new BadRequestError(
          `Cannot return ${item.quantity} units of '${saleItem.product.name}'. Remaining returnable quantity: ${remainingReturnable}`
        );
      }
    ...
    let commissionReversal: AtomicReturnPlan['commissionReversal'] = null;

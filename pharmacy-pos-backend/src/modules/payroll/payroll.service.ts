...
function formatPayroll(raw: any): PayrollResponse {
...
export class PayrollService {
  ...
  async calculateCommissionForPeriod(userId: string, startDate: Date, endDate: Date): Promise<{
    earned: number;
    reversed: number;
    net: number;
  }> {
  ...
  async generatePayroll(input: GeneratePayrollDTO, actorId: string): Promise<PayrollResponse> {
    ...
    if (existing) {
      throw new ConflictError(
        `Active payroll already exists for '${user.name}' in the period ${startDate.toISOString().slice(0, 10)} to ${endDate.toISOString().slice(0, 10)} (ID: ${existing.id}, Status: ${existing.status})`
      );
    }
  ...
  async generatePeriodPayroll(input: GeneratePeriodPayrollDTO, actorId: string): Promise<PayrollResponse[]> {
    const { startDate, endDate } = parseDateRange(input.periodStart, input.periodEnd);
    const allUsers = await this.users.getUsers({ limit: 100, isActive: true });

    const salaryMap = new Map<string, { baseSalary: number; bonus?: number; deductions?: number }>();
    ...
    const generated: PayrollResponse[] = [];
  ...
  async getPayrolls(filters: PayrollQueryDTO): Promise<PaginatedPayrollResponse> {
    const page = Math.max(1, Number(filters.page) || 1);
    const limit = Math.max(1, Number(filters.limit) || 20);

    const queryFilters: PayrollQueryFilters = {
      page,
      limit,
  ...
  async getPayrollById(id: string): Promise<PayrollResponse> {
    const payroll = await this.repo.findById(id);
    if (!payroll) {
      throw new NotFoundError(`Payroll record with ID '${id}' not found`);
    }

    const comm = await this.calculateCommissionForPeriod(
      payroll.userId,
  ...
  async getEmployeePayrolls(employeeId: string): Promise<PayrollResponse[]> {
    const payrolls = await this.repo.findByEmployeeId(employeeId);
    return payrolls.map(formatPayroll);
  }

  async updatePayroll(id: string, input: UpdatePayrollDTO, actorId: string): Promise<PayrollResponse> {
  ...
  async approvePayroll(id: string, actorId: string): Promise<PayrollResponse> {
    const existing = await this.repo.findById(id);
    if (!existing) {
      throw new NotFoundError(`Payroll record with ID '${id}' not found`);
    }
  ...
  async payPayroll(id: string, input: PayPayrollDTO, actorId: string): Promise<PayrollResponse> {
    const paid = await this.repo.payPayrollAtomic(
      id,
  ...
  async cancelPayroll(id: string, actorId: string): Promise<PayrollResponse> {
    const existing = await this.repo.findById(id);
    if (!existing) {
      throw new NotFoundError(`Payroll record with ID '${id}' not found`);
    }
  ...
  async getSummary(from?: string, to?: string): Promise<PayrollSummaryResponse> {

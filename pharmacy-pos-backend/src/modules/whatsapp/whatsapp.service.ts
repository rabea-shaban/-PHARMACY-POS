import { whatsAppRepository, WhatsAppRepository } from './whatsapp.repository.js';
import { defaultWhatsAppProvider, MockWhatsAppProvider } from './whatsapp.provider.js';
import { notificationsService, NotificationsService } from '../notifications/notifications.service.js';
import { auditService, AuditService } from '../audit/audit.service.js';
import { eventBus, SaleCompletedEventPayload } from '../../lib/events.js';
import { getPaginationMeta } from '../../utils/pagination.util.js';
import {
  WhatsAppMessageResponse,
  EnqueueInvoiceMessageInput,
...
function formatWhatsAppMessage(raw: any): WhatsAppMessageResponse {
  return {
    id: raw.id,
...
export class WhatsAppService {
  constructor(
    private readonly repo: WhatsAppRepository = whatsAppRepository,
    private readonly provider: IWhatsAppProvider = defaultWhatsAppProvider,
  ...
  async enqueueInvoiceMessage(input: EnqueueInvoiceMessageInput): Promise<WhatsAppMessageResponse | null> {
  ...
  async processMessageWithRetries(messageId: string, maxAttempts: number = 3): Promise<void> {
  ...
  async retryMessage(id: string, actorId: string): Promise<WhatsAppMessageResponse> {
  ...
  async getMessages(filters: WhatsAppQueryDTO): Promise<PaginatedWhatsAppMessagesResponse> {
  ...
  async getMessageById(id: string): Promise<WhatsAppMessageResponse> {

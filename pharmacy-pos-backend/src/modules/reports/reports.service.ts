...
export class ReportsService {
  ...
  // 1. Sales Report
  async getSalesReport(filters: SalesReportQueryFilters): Promise<SalesReportResponse> {
    ...
    const pmMap = new Map<string, { paymentMethod: any; amount: number; count: number }>();
  ...
  async getProductPerformanceReport(filters: ProductReportQueryFilters): Promise<ProductReportResponse> {
    const { startDate, endDate, fromStr, toStr } = parseDateRange(filters.from, filters.to);
    ...
    const returnsMap = new Map<string, { quantity: number; refund: number }>();
  ...
  // 3. Inventory Report
  async getInventoryReport(filters: InventoryReportQueryFilters): Promise<InventoryReportResponse> {
    const { startDate, endDate, fromStr, toStr } = parseDateRange(filters.from, filters.to);
    ...
    const lowStockItems: InventoryReportResponse['lowStockItems'] = [];
    for (const p of data.products) {
      const currentStock = p.batches.reduce((sum, b) => sum + b.quantity, 0);
    ...
    const movementMap = new Map<string, { type: string; totalQuantity: number; transactionCount: number }>();
  ...
  // 4. Purchase Report
  async getPurchaseReport(filters: PurchaseReportQueryFilters): Promise<PurchaseReportResponse> {
    const { startDate, endDate, fromStr, toStr } = parseDateRange(filters.from, filters.to);
    ...
    const supplierMap = new Map<string, { supplierId: string; supplierName: string; supplierPhone: string; invoiceCount: number; totalSpent: number }>();
    const monthMap = new Map<string, { month: string; invoiceCount: number; totalAmount: number }>();
    ...
    for (const p of data.purchases) {
      ...
      if (p.status === 'RECEIVED') receivedInvoices++;
      else if (p.status === 'PENDING') pendingInvoices++;
      else if (p.status === 'CANCELLED') cancelledInvoices++;
  ...
  // 5. Expense Report
  async getExpenseReport(filters: ExpenseReportQueryFilters): Promise<ExpenseReportResponse> {
    const { startDate, endDate, fromStr, toStr } = parseDateRange(filters.from, filters.to);
    const data = await this.repo.getExpenseReportData(filters, startDate, endDate);

    let totalExpenses = 0;
    const catMap = new Map<any, { category: any; amount: number; count: number }>();
    const pmMap = new Map<any, { paymentMethod: any; amount: number; count: number }>();
  ...
  async getCustomerReport(filters: CustomerReportQueryFilters): Promise<CustomerReportResponse> {
    const { startDate, endDate, fromStr, toStr } = parseDateRange(filters.from, filters.to);
    ...
    for (const lt of data.loyaltyTransactions) {
      if (lt.type === 'EARN') pointsEarned += Math.abs(lt.points);
      else if (lt.type === 'REDEEM') pointsRedeemed += Math.abs(lt.points);
    }

    let totalCustomerSpend = 0;
    const customerPerformance: CustomerReportResponse['topCustomersByRevenue'] = [];
  ...
  async getStaffReport(filters: StaffReportQueryFilters): Promise<StaffReportResponse> {
    const { startDate, endDate, fromStr, toStr } = parseDateRange(filters.from, filters.to);
    const data = await this.repo.getStaffReportData(filters, startDate, endDate);

    const salesPerUser = new Map<string, { count: number; amount: number }>();
    ...
    const commPerUser = new Map<string, { earned: number; reversed: number }>();
  ...
  async getFinancialSummary(from?: string, to?: string): Promise<FinancialSummaryResponse> {

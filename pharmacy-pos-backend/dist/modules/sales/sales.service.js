...
export class SalesService {
    ...
    async checkout(input, cashierId) {
        let customer = null;
        if (input.customerId) {
        ...
        for (const item of input.items) {
            ...
            if (!fefoResult.fulfilled) {
                ...
                throw new BadRequestError(`Insufficient eligible stock for product '${product.name}'. Required: ${item.quantity}, Available non-expired: ${availableNonExpired}`);
            }
        ...
        if (discountIdentifier) {
            const promoDiscount = await this.discounts.validateAndCalculateDiscount(discountIdentifier, subtotal - totalDiscount);
        ...
        if (input.customerInsuranceId) {
            ...
            const insuranceCalculation = await this.insurance.validateAndCalculateInsurance(input.customerInsuranceId, input.customerId, discountedAmount);
            insurancePlan = {
        ...
        if (Math.abs(roundedPayments - payableByCustomer) > 0.01) {
            throw new BadRequestError(`Payment total (${roundedPayments} EGP) does not match the payable amount (${payableByCustomer} EGP)`);
        }
    ...
    async cancelSale(id, actorId, input) {

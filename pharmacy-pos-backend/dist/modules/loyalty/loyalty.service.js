...
export class LoyaltyService {
    ...
    async getLoyaltyTransactions(customerId, filters) {
        const account = await this.repo.findByCustomerId(customerId);
        if (!account) {
            throw new NotFoundError(`Loyalty account for customer ID '${customerId}' not found`);
        }
    ...
    async earnPoints(customerId, input, actorId) {
    ...
    async redeemPoints(customerId, input, actorId) {
        const result = await this.repo.applyPointsTransactionAtomic({
            customerId,
            pointsDelta: -input.points,
            type: 'REDEEM',
    ...
    async adjustPoints(customerId, input, actorId, actorRole) {

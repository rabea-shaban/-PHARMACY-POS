...
export class DiscountsService {
    ...
    async validateAndCalculateDiscount(discountIdOrCode, subtotal) {
        ...
        if (minPurchase > 0 && subtotal < minPurchase) {
            throw new BadRequestError(`Minimum purchase amount of ${minPurchase} EGP required for discount '${discount.name}' (current subtotal: ${subtotal} EGP)`);
        }
    ...
    async createDiscount(input, actorId) {
        if (input.code) {
    ...
    async updateDiscount(id, input, actorId) {
        ...
        if (input.code !== undefined)
            updateData.code = input.code ? input.code.trim().toUpperCase() : null;
        if (input.name)
        ...
        if (input.minimumPurchase !== undefined)
            updateData.minimumPurchase = input.minimumPurchase;
    ...
    async deleteDiscount(id, actorId) {
        const existing = await this.repo.findById(id);
        if (!existing) {
            throw new NotFoundError(`Discount with ID '${id}' not found`);
        }

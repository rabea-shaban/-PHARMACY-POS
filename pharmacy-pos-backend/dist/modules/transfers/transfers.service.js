...
export class TransfersService {
    ...
    async createTransfer(data, actorId) {
        if (!actorId) {
    ...
    async approveTransfer(id, actorId) {
        if (!actorId) {
        ...
        if (user &&
            ...
            throw new ForbiddenError('الموافقة على طلب التحويل وقبوله يجب أن تتم من خلال الصيدلية / الفرع المستلم');
        }
    ...
    async dispatchTransfer(id, actorId) {
        if (!actorId) {
    ...
    async receiveTransfer(id, actorId) {
        if (!actorId) {
    ...
    async rejectTransfer(id, input, actorId) {
        const existing = await this.repo.findById(id);
        if (!existing) {
            throw new NotFoundError(`Transfer request with ID '${id}' not found`);
        }
    ...
    async cancelTransfer(id, actorId) {
        const existing = await this.repo.findById(id);
        if (!existing) {
            throw new NotFoundError(`Transfer request with ID '${id}' not found`);
        }
